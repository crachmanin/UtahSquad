...
def quest_type(sent):
    #not sure about the category classification for "which"
    person = set([u'who', u'whom', u'whose'])
    place = set([u'where'])
    time = set([u'when'])
    quantity = set([u'few', u'little', u'much', u'many'])
    age = set([u'young', u'old', u'long'])
    quest_type = 0

    for word in sent[0:2]:    #Remove index to check entire sentence instead of first two word
        ...
        if word.lower() in time:
            quest_type = 3
        if word.lower() == u'how':
            if word.lower() in quantity:
                quest_type = 4
            if word.lower() in age:
                quest_type = 3
    return MAX_SENTENCES * 20 + MAX_SENTENCES * 3 + quest_type
    ...
    # for i in xrange(1):
        for pgraph in topic['paragraphs']:
            ...
            for qa in pgraph['qas']:
                ...
                q_type = quest_type(question_words)
...
def main():
    ...
    with open("word_features.txt", 'w') as train_fp:
