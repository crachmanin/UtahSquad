...
def left_right_in_question(np, sent_num, sent_boundaries, c_parsed,
    ...
    left_words = [c_parsed[np.start - i].orth_.lower() for i in range(1, 6)
    ...
    right_words = [c_parsed[np.end + i].orth_.lower() for i in range(5)
...
def left_right_context(np, sent_num, sent_boundaries, c_parsed, q_parsed, features):
    ...
    left_words = [c_parsed[np.start - i] for i in range(1, 6)
                  if np.start - i >= sent_boundaries[sent_num][0]]
    for i, q_word in enumerate(q_parsed[:3]):
        q_word = q_word.orth_
    ...
    right_words = [c_parsed[np.end + i] for i in range(5)
                   if np.end + i < sent_boundaries[sent_num][1]]
    for i, q_word in enumerate(q_parsed[:3]):
        q_word = q_word.orth_
...
def libsvm_features(label, features):
    features = sorted(set([feat for feat in features if feat]))
